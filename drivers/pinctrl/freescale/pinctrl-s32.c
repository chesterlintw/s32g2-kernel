 ...
 * @mscr_base: the offset to the mscr register in virtual memory
 * @imcr_base: the offset to the imcr register in virtual memory
 * @info: structure containing informations about the pin
				...
				struct s32_pinctrl_soc_info *info)
{
	...
	s32_pinctrl_parse_functions(np, info, 0);
		      ...
		      struct s32_pinctrl_soc_info *info)
{
	...
	dev_info(&pdev->dev, "Add generic pinmux info.\n");

	for (i = 0; i < info->ngroups; i++) {
		dev_info(&pdev->dev, "Add generic group: %s, npins: %u",
			 info->groups[i].name,
			 info->groups[i].npins);
		pinctrl_generic_add_group(ipctl->pctl, info->groups[i].name,
					  info->groups[i].pin_ids,
					  info->groups[i].npins, NULL);

	}

	dev_info(&pdev->dev, "Add generic func: %s, ngroups: %u\n",
		 info->functions[0].name, info->ngroups);
	pinmux_generic_add_function(ipctl->pctl, info->functions[0].name,
				    info->functions[0].groups,
				    info->ngroups, NULL);
