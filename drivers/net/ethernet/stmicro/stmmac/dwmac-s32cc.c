...
/*
 * NXP S32 GMAC glue layer based on stmmac.
 *
...
#define GMAC_CLK_125M	125000000	/* 125MHz */
#define GMAC_CLK_25M	25000000	/* 25MHz */
#define GMAC_CLK_2M5	2500000		/* 2.5MHz */
...
static void s32cc_fix_speed(void *priv, unsigned int speed)
{
	...
	if (gmac->interface == PHY_INTERFACE_MODE_SGMII)
		return;

	switch (speed) {
			case SPEED_1000:
				dev_info(gmac->dev, "Set RX/TX clock to 125M\n");
				clk_set_rate(gmac->tx_clk, GMAC_CLK_125M);
				clk_set_rate(gmac->rx_clk, GMAC_CLK_125M);
				break;
			case SPEED_100:
				dev_info(gmac->dev, "Set RX/TX clock to 25M\n");
				clk_set_rate(gmac->tx_clk, GMAC_CLK_25M);
				clk_set_rate(gmac->rx_clk, GMAC_CLK_25M);
				break;
			case SPEED_10:
				dev_info(gmac->dev, "Set RX/TX clock to 2.5M\n");
				clk_set_rate(gmac->tx_clk, GMAC_CLK_2M5);
				clk_set_rate(gmac->rx_clk, GMAC_CLK_2M5);
				break;
			default:
...
static int s32cc_dwmac_probe(struct platform_device *pdev)
{
	...
	/* phy mode */
	ret = of_get_phy_mode(pdev->dev.of_node, &gmac->interface);
	if (ret) {
